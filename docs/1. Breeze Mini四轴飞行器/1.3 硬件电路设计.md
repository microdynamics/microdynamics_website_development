...
这部分将会详细介绍 **Breeze Mini 1** 的硬件电路设计，该电路是[myyerrol](https://github.com/myyerrol)负责设计的，他在其博客中有下面的一段介绍：

  > 我曾经在大二的时候为完成数字电路实验而简单地学过一些硬件电路设计，当时我看的是郭天祥老师的视频教程，里面有关Altium Designer 6.9的操作可以说是讲得非常细致，但是由于后来临近期末考试，时间较为紧迫，而且实验电路本身并不复杂，所以我只学了其中有关PCB布线的知识。

  > 可能是之前我还有一些PCB设计的基础，所以我在这次设计Breeze Mini微型四轴飞行器PCB的过程中感觉还是比较容易上手的。我最早是在Ubuntu 14.04上用[KiCad](http://kicad-pcb.org)来设计硬件(因为Crazyflie 1.0四轴飞行器的PCB就是用KiCAD画的)，但是后来综合考虑到KiCAD在处理复杂元器件布线以及规则约束方面的短板，所以最终我们还是决定使用功能更加强大的Altium Designer 16来重新完成对PCB的绘制。

## 概述

**Breeze Mini 1** 微型四轴飞行器硬件电路主要由五个部分组成，以下是硬件的架构图：
...
电机驱动模块主要用来接收主控制器发送过来的PWM控制信号，通过CMOS三极管将控制信号进行放大，进而实现对四轴飞行器每个空心杯电机的转速大小的控制。
...
传感器模块包括了陀螺仪、加速度计、磁力计和气压计。正是由以上这些传感器所构成的10自由度IMU **(惯性测量单元)** 为四轴飞行器提供了绝对或相对的姿态和位置信息，才使得Breeze Mini可以在飞行过程中获得自身的运动状态。

**无线通信模块**

无线通信模块根据预先制定好的通信协议，在四轴飞行器和远程PC端、四轴飞行器和手机APP上位机之间建立起了数据通信链路，便于对飞行器的运动状态进行监视和控制。

**主控制器模块**

主控制器模块是四轴飞行器的核心，它主要接收从传感器模块和无线通信模块发送过来的数据，并根据不同的控制命令对姿态数据进行数学解算、融合，最后将解算结果以PWM信号的形式发送给电机驱动模块。
...
* **成果**
    ...
    这里摘录一段[myyerrol](https://github.com/myyerrol)在设计 **Breeze Mini 1** 初代硬件过程中的个人总结：

    > 下面的这张图片是我在Ubuntu 14.04上用KiCad软件所设计的第一代Breeze Mini 1微型四轴飞行器的PCB图，由于KiCad软件不像Altium Designer那样带有 **IC封装生成向导** 的功能，所以当时我是边看IC的Datasheet边用KiCad的PCB库编辑功能来进行封装的绘制，这个过程很花费时间，不过我也确实学到了一些PCB封装设计方面的经验和技巧 **(比如根据人工焊或机焊方式的不同来修改IC四周引脚的长、宽度等)**。最后很遗憾的是，由于我们对Breeze Mini 1微型四轴飞行器的功能需求进行了较大的变更并考虑到后期PCB打板方面的问题，因此我们决定放弃KiCad转而使用Altium Designer来完成PCB电路板的设计工作(这也是为什么下图中的PCB没有完成的原因)，但有一点我不得不说，作为一个开源且跨平台的EDA软件，KiCad真的很好上手，而且它在国外Hack界拥有非常高的人气，毕竟[Crazyflie](https://www.bitcraze.io)和[Pixhawk](https://pixhawk.org)两大知名的开源项目都曾使用KiCad作为其PCB设计的工具。
...
* **简介**
    ...
    [Altium Designer](http://www.altium.com)是原Protel软件开发商Altium公司推出的一款EDA软件，它集成了原理图设计、电路仿真、PCB绘制、拓扑逻辑自动布线、信号完整性分析和设计输出等功能。除此之外，Altium Designer设计平台还拓宽了板级设计的传统界面，全面集成了FPGA和SOPC设计，为设计者提供了全新的一体化解决方案，提高了电路设计的质量和效率。

* **成果**

    下图所展示的就是最终我们用Altium Designer所完成的 **Breeze Mini 1** 的PCB文件，该PCB采用双面布局走线、双面覆铜处理、四层电路板设计。除此之外，我们还对连接有直流电机器件的电源线路进行了加粗处理，以提高对电机的驱动能力；还有就是由于该PCB使用了陶瓷天线 **(下图最中间上面那个突出的长方形区域)**，为了减少电源层对其信号传输的干扰，我们对四层电路板中的VCC和GND层进行了分割处理，即将陶瓷天线元件下方的VCC和GND层掏空，这样电流不从陶瓷天线的底部通过，这样可降低电流所产生的电磁干扰对天线的影响。最后总结一下，Altium Designer这款EDA软件是我们用过的PCB设计工具 **(其它有KiCad和Cadence SPB)** 中对初学者最友好的，原因主要是其拥有美观的人机交互界面、便捷强大的设计功能和丰富的教程案例。
